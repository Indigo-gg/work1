...
class ImageSpectrumClassifier:
    ...
    def extract_spectrum_features(self, image_path):
        ...
        try:
            ...
            # 组合特征：[slope, dc_component, bin_mag_mean, spectrum_entropy, symmetry]
            features = np.array([
    ...
    def batch_extract_features(self, image_paths, batch_size=100):
        """批量提取特征（修复tqdm调用问题）"""
        from tqdm import tqdm  # 明确导入tqdm函数
        
        all_features = []
        n_images = len(image_paths)
        
        with ThreadPoolExecutor(max_workers=self.n_workers) as executor:
            results = list(tqdm(executor.map(self.extract_spectrum_features, image_paths), 
    ...
    def preprocess_data(self, force_extract=False):
        """预处理数据：从txt读取并提取特征，支持加载已处理特征"""
        # 检查是否已存在处理好的特征
        if (not force_extract and 
            os.path.exists(self.train_features_path) and
            os.path.exists(self.test_features_path)):
            
            print(f"加载已处理的特征数据: {self.dataset_dir}")
    ...
    def predict(self, image_paths):
        ...
        # 提取特征
        features, _ = self.batch_extract_features(image_paths)
        if len(features) == 0:
            return np.array([])
...
if __name__ == "__main__":
    # 命令行参数
    parser = argparse.ArgumentParser(description="图像频谱分类器")
    ...
    # 初始化分类器
    classifier = ImageSpectrumClassifier(
    ...
    # 预处理数据
    X_train, y_train, X_test, y_test = classifier.preprocess_data(force_extract=args.force_extract)
    
    # 训练模型
    accuracy = classifier.train_xgboost(X_train, y_train, X_test, y_test)
    
    # 保存模型
    classifier.save_model()
    
    print(f"训练完成，测试集准确率: {accuracy:.4f}")
    print(f"特征数据保存在: {classifier.dataset_dir}")
    # 命令行参数
    parser = argparse.ArgumentParser(description="图像频谱分类器")
    parser.add_argument("--train_file", default="./dataset/train_list.txt", help="训练集txt文件")
    parser.add_argument("--test_file", default="./dataset/test_list.txt", help="测试集txt文件")
